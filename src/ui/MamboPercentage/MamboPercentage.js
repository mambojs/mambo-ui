...
ui.class.MamboPercentage = class MamboPercentage extends HTMLElement {
  constructor(initOptions) {
    ...
    if (!initOptions.parentTag) {
      console.error(
        `Percentage: initOptions.parentEle parameter not passed in.`
      );
      return;
    }
    ...
    function setup() {
      ...
      if (!m_parentTag) {
        console.error(
          `Percentage: dom. parent tag ${initOptions.parentTag} was not found.`
        );
        return;
      }
    ...
    function setText() {
      m_percentageTextTag.innerText = m_utils.formatPercentage(
        m_value,
        m_config.decimals
      );
    }
