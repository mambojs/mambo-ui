...
ui.class.MamboDraggable = class MamboDraggable extends HTMLElement {
  constructor(parentTag, containerTag, options) {
    ...
    function dragStart(ev) {
      if (m_enable) {
        m_bounding = m_containerTag
          ? m_containerTag.getBoundingClientRect()
          : null;
    ...
    function drag(ev) {
      if (m_enable && m_active) {
        ...
        if (m_bounding) {
          clientX = Math.max(
            m_bounding.left,
            Math.min(clientX, m_bounding.right)
          );
          clientY = Math.max(
            m_bounding.top,
            Math.min(clientY, m_bounding.bottom)
          );
        }
        ...
        if (Array.isArray(m_config.grid) && m_config.grid.length === 2) {
          if (m_bounding) {
            currentX = getAxisStep(
              currentX,
              m_config.grid[0],
              m_bounding.left - m_initialX,
              m_bounding.right - m_initialX
            );
            currentY = getAxisStep(
              currentY,
              m_config.grid[1],
              m_bounding.top - m_initialY,
              m_bounding.bottom - m_initialY
            );
          } else {
