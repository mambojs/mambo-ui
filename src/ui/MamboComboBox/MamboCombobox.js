...
ui.class.MamboCombobox = class MamboCombobox extends HTMLElement {
  constructor(initOptions) {
    ...
    if (!initOptions.parentTag) {
      console.error(
        `ComboBox: initOptions.parentTag parameter was not passed in.`
      );
      return;
    }
    ...
    function setup() {
      ...
      if (!m_parentTag) {
        console.error(
          `ComboBox: dom. parent tag ${initOptions.parentTag} was not found.`
        );
        return;
      }
    ...
    function installDropdown() {
      ...
      dropdown.fnBeforeClose = (context) => {
        const result = m_config.dropdown.fnBeforeClose
          ? m_config.dropdown.fnBeforeClose(context)
          : true;
        return (
          (!context.ev || !m_input.getTag().contains(context.ev.target)) &&
          result
        );
      };
    ...
    function installButtonGroup(dropdown, data) {
      if (!data || !Array.isArray(data)) {
        console.error(
          "Data ComboBox alert: combobox data not found or is not data type Array -->",
          parentTag
        );
        return;
      }
    ...
    function filterItems() {
      if (m_config.filter) {
        const data = m_string.filterArray(
          m_comboBoxData,
          m_input.value(),
          getItemDataText,
          "contains"
    ...
    function setValue(value, ev) {
      ...
      const item = m_string.findInArray(
        m_comboBoxData,
        value,
        getItemDataText,
        "equals"
