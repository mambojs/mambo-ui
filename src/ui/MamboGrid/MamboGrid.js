...
ui.grid = class MamboGrid extends HTMLElement {
  constructor(initOptions) {
    ...
    function setup() {
      ...
      if (!m_parentTag) {
        console.error(
          `Data Grid: dom. parent tag ${initOptions.parentTag} was not found.`
        );
        return;
      }
    ...
    function installGrid() {
      ...
      dom
        .append(m_gridParentTag, m_gridHdrTag)
        .append(m_gridParentTag, m_gridBodyTag);
    ...
    function installHdr() {
      m_config.columns.forEach((column) => {
        let parentTag = dom.createTag(m_config.tag.colCell, {
    ...
    async function installRows() {
      ...
      function processRow(rowData, rowIndex) {
        ...
        m_config.columns.forEach((column, colIndex) => {
          // Create cell parent
          const parentTag = dom.createTag(m_config.tag.colCell, {
            class: m_config.css.colCell,
          });
          applyColCellElStyles(column, parentTag);
    ...
    function installTextCell(context) {
      const text =
        context.column.dataKey in context.rowData
          ? context.rowData[context.column.dataKey]
          : context.column.text;
      const tagConfig = {
    ...
    function installFileChooserCell(context) {
      ...
      const fileChooserTag = new ui.fileChooser(chooserConfig);
      addComponentToMap({
        column: context.column,
        colIndex: context.colIndex,
    ...
    function installDialogCell(context) {
      ...
      const dialogConfig = m_utils.extend(
        true,
        dialogDefaultConfig,
        context.column
      );
    ...
    function installButtonGroupCell(context) {
      // Extend with header configuration
      let buttonGroupConfig = m_utils.extend(
        true,
        { css: { button: m_config.css.button } },
        context.column
      );
      buttonGroupConfig.id = context.rowIndex;

      const buttonGroupTag = new ui.buttonGroup(
        context.parentTag,
        buttonGroupConfig
      );
      addComponentToMap({
    ...
    function installDropdownCell(context) {
      ...
      const config = m_utils.extend(true, defaultConfig, context.column);
      config.parentTag = context.parentTag;
      const dropdownTag = new ui.dropdown(config);
    ...
    function installComboboxCell(context) {
      ...
      const config = m_utils.extend(true, defaultConfig, context.column);
      config.parentTag = context.parentTag;
      const comboboxTag = new ui.combobox(config);
    ...
    function installTimePickerCell(context) {
      ...
      const config = m_utils.extend(true, defaultConfig, context.column);
      config.parentTag = context.parentTag;
      const timePickerTag = new ui.timePicker(config);
    ...
    function installDatePickerCell(context) {
      ...
      const config = m_utils.extend(true, defaultConfig, context.column);
      config.parentTag = context.parentTag;
      const datePickerTag = new ui.datePicker(config);
    ...
    function saveCellTagWidth(context) {
      ...
      m_colsMaxPxWidth[context.colIndex] =
        tagWidth > m_colsMaxPxWidth[context.colIndex]
          ? tagWidth
          : m_colsMaxPxWidth[context.colIndex];
    }
    ...
    function setColsWidth() {
      ...
      m_colsMaxPxWidth.forEach((width, index) => {
        ...
        dom.append(
          styleEl,
          document.createTextNode(`${hdrSelector},${bodySelector}${style}`)
        );
      });
    ...
    function validateGridData() {
      if (!m_gridData || !Array.isArray(m_gridData)) {
        console.error(
          "Data Grid alert: grid data not found or is not data type Array -->",
          parentTag
        );
        finishSetup();
        return false;
...
customElements.define("mambo-grid", ui.grid);
