  ...
  constructor(initOptions) {
    ...
    function appendInputElement(parent) {
      ...
      if (m_config.maxLenWidth && !m_config.leftSide && !m_config.rightSide) {
        const width = `${m_config.attr.maxLength + m_config.maxLenWidthAdj}${
          m_config.maxLenWidthUnit
        }`;
    ...
    function validateMinLength(config, ev) {
      ...
      if (typeof config.value === "string") {
        ...
        if (length > 0) {
          ...
          m_inputTag.value =
            config.dir === "right"
              ? m_inputTag.value + padding
              : padding + m_inputTag.value;
        ...
        console.error(
          `Input() or Grid() tag: validate Input field using minLength requires a STRING value for the property key VAL`
        );
      }
    }
