...
ui.class.MamboSlider = class MamboSlider extends HTMLElement {
  constructor(initOptions) {
    ...
    function setup() {
      ...
      if (!m_parentTag) {
        console.error(
          `Slider: dom. parent tag ${initOptions.parentTag} was not found.`
        );
        return;
      }
    ...
    function installTags() {
      ...
      if (m_config.showButtons) {
        if (m_horizontal) {
          installButton(
            m_config.decreaseButton,
            m_css.decreaseButton,
            decrease
          );
        } else {
          installButton(
            m_config.increaseButton,
            m_css.increaseButton,
            increase
          );
        }
      }
    ...
    function installSteps() {
      ...
      const trackLength = m_horizontal
        ? m_trackTag.clientWidth
        : m_trackTag.clientHeight;
    ...
    function getSelectedIndex() {
      let handleOffset = m_horizontal
        ...
        : m_sliderWrapperTag.clientHeight -
          m_handleTag.getParentTag().offsetTop;

      for (let i = 0; i < m_stepTags.length; i++) {
        let stepOffset = m_horizontal
          ? m_stepTags[i].offsetLeft
          : m_sliderWrapperTag.clientHeight - m_stepTags[i].offsetTop;
    ...
    function setEnable(enable) {
      m_enable = enable;
      m_enable
        ? dom.removeClass(m_sliderParentTag, m_config.css.disabled)
        : dom.addClass(m_sliderParentTag, m_config.css.disabled);
    ...
    function setHandlePosition() {
      ...
      if (m_horizontal) {
        const length =
          stepTag.getBoundingClientRect().left -
          m_sliderWrapperTag.getBoundingClientRect().left -
          handleTag.clientWidth / 2;
        handleTag.style.left = length + "px";
      } else {
        const length =
          stepTag.getBoundingClientRect().top -
          m_sliderWrapperTag.getBoundingClientRect().top -
          handleTag.clientHeight / 2;
    ...
    function setSelectionPosition() {
      ...
      if (m_horizontal) {
        ...
        const length =
          m_sliderWrapperTag.getBoundingClientRect().bottom -
          handleTag.getBoundingClientRect().bottom;
