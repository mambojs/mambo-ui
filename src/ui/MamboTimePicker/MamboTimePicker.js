...
ui.timePicker = class MamboTimePicker extends HTMLElement {
  constructor(initOptions) {
    ...
    if (!initOptions.parentTag) {
      console.error(
        `TimePicker: initOptions.parentTag parameter was not passed in.`
      );
      return;
    }
    ...
    function setup() {
      ...
      if (!m_parentTag) {
        console.error(
          `TimePicker: dom. parent tag ${initOptions.parentTag} was not found.`
        );
        return;
      }
    ...
    function createComboBoxData() {
      ...
      return m_dateMgr.createInterval(
        m_config.interval,
        "m",
        min,
        max,
    ...
    function selectTime(context) {
      m_value = context.button
        ? m_dateMgr.createDate(context.button.text(), m_config.format)
        : null;
