...
ui.class.MamboRating = class MamboRating extends HTMLElement {
  constructor(initOptions) {
    ...
    function setup() {
      ...
      if (!m_parentTag) {
        console.error(
          `Rating: dom. parent tag ${initOptions.parentTag} was not found.`
        );
        return;
      }
    ...
    function setHoverValue(ev) {
      if (m_enable) {
        ...
        m_ratingHoverTag.style.width =
          getStarWidth() * getHoverValue(ev) + "px";
      }
    }
    ...
    function setEnable(enable) {
      m_enable = enable;
      m_enable
        ? dom.removeClass(m_ratingParentTag, m_config.css.disabled)
        : dom.addClass(m_ratingParentTag, m_config.css.disabled);
    }
