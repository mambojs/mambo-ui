(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["HlsjsIpfsLoader"] = factory();
	else
		root["HlsjsIpfsLoader"] = factory();
})(window, function() {
...
class HlsjsIPFSLoader {
  ...
  loadInternal() {
    ...
    stats.loaded = 0; //When using absolute path (https://example.com/index.html) vs https://example.com/
...
async function getFile(ipfs, rootHash, filename, options, debug, abortFlag) {
  debug("Fetching hash for '".concat(rootHash, "/").concat(filename, "'"));
  const path = "".concat(rootHash, "/").concat(filename);

  try {
    return await cat(path, options, ipfs, debug, abortFlag);
  } catch (ex) {
    throw new Error("File not found: ".concat(rootHash, "/").concat(filename));
  }
}
...
async function cat(cid, options, ipfs, debug, abortFlag) {
  ...
  debug("Received data for file '".concat(cid, "' size: ").concat(value.length, " in ").concat(parts.length, " blocks"));
  return value;
}
